...
deficits_data_pb <- deficits_data |> 
  ...
  group_by(geo) |> 
  mutate(consolid = value[annee == 2025] - value[annee == 2023],
         endpoint = value[annee == 2025],
         value = case_when(annee == 2023 ~ value[annee==2023]-value[annee==2019],
                           TRUE ~ value),
         across(value:endpoint, ~ round(., digits = 1))) |>
  filter(annee%in% c(2019,2023))
...
ggplot(deficits_data_pb,
       aes(x=geo, y = value, fill = annee))+
  geom_bar(stat="identity", position = "stack")#+
  geom_point(aes(x=geo, y = endpoint))
...
graph_deficits_pbinter <-
  (ggplot(deficits_data_pb |> drop_na(),
    ...
    geom_point_interactive(aes(y=d_2025, data_id=d_2025, tooltip= str_c("Déficit 2025 (prévision) : <br>", d_2025)), size = 3)+
    xlab("")+
    ylab("% du PIB")
